...
window.onload = () => {
  ...
  const bubbleSort = arr => {
    ...
    do {
      swapped = false;
      for (let i = 0; i < n - 1; i++) {
        const numberA = cardOrder.indexOf(arr[i].number);
        const numberB = cardOrder.indexOf(arr[i + 1].number);
        if (numberA > numberB) {
  ...
  clasificacionButton.addEventListener("click", () => {
    /* Ordenar las cartas usando el algoritmo Bubble Sort */
    const sortedCards = bubbleSort(cardArray);
